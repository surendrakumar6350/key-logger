...
function useGlobalSearch() {
  ...
  // Available months for filtering - normally you'd fetch these from an API
...
export default function GlobalSearchPage() {
  ...
  const handleMonthChange = (value: string) => {
    search({ newMonth: value === 'all' ? undefined : value, newPage: 1 });
  };
            ...
            <div className="w-full md:w-[240px]">
              <Select value={month || 'all'} onValueChange={handleMonthChange}>
                ...
                <SelectContent className="bg-gray-800 border-gray-700 text-white">
                  <SelectItem value="all" className="focus:bg-gray-700 focus:text-white">All dates</SelectItem>
                  {availableMonths.map((m) => (
                    <SelectItem key={m.value} value={m.value} className="focus:bg-gray-700 focus:text-white">{m.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Input type="date" className="bg-gray-800 border-gray-700" placeholder="From" />
                        <Input type="date" className="bg-gray-800 border-gray-700" placeholder="To" />
                      </div>
                    </div>
                    
                    <div className="space-y-2">
                      <label className="text-sm font-medium">User</label>
                      <Input className="bg-gray-800 border-gray-700" placeholder="Filter by user" />
                    </div>
                    
                    <div className="space-y-2">
                      <label className="text-sm font-medium">Source</label>
                      <div className="flex items-center space-x-2">
                        <Checkbox id="database" className="bg-gray-800 border-gray-700" />
                        <label htmlFor="database" className="text-sm">Database</label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Checkbox id="archive" className="bg-gray-800 border-gray-700" />
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            log.source === 'database' ? 'bg-blue-600/20 text-blue-300' : 'bg-green-600/20 text-green-300'
                          }`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {results.map((log, index) => (
                <Card 
                  key={log._id || `${log.timestamp}-${index}`} 
                  className={`cursor-pointer hover:bg-gray-800 transition-colors bg-gray-900 border border-gray-800 ${
                    selectedResult === log ? 'ring-2 ring-blue-500' : ''
                  }`}
                  onClick={() => {
                    setSelectedResult(log);
                    setShowDetailPanel(true);
